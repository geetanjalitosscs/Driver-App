...
class CentralizedApiService {
  ...
  /// Login API
  static Future<Map<String, dynamic>> login(String email, String password, {double? latitude, double? longitude, String? address, String? deviceId, String? deviceName}) async {
    try {
      final requestBody = {
        'email': email,
        'password': password,
      ...
      // Debug logging
      print('🔍 API Debug - Request body: $requestBody');
  ...
  /// Logout API
  static Future<Map<String, dynamic>> logout(int driverId, String deviceId) async {
    try {
      final response = await http.post(
        Uri.parse(ApiEndpoints.getAccidentUrl('logout.php')),
        headers: {'Content-Type': 'application/json'},
        body: json.encode({
          'driver_id': driverId,
          'device_id': deviceId,
